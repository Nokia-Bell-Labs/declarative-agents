...
func RegisterEvalFactories(br *toolregistry.BuiltinRegistry, deps EvalFactoryDeps) {
	...
	br.Register("parse_suite_config", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
	...
	br.Register("discover_suite_samples", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := evaluatorSessionConfigFactory(es, func(es *EvalSessionState) core.Builder {
	...
	br.Register("expand_eval_grid", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := evaluatorSessionConfigFactory(es, func(es *EvalSessionState) core.Builder {
	...
	br.Register("init_eval_session", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := evaluatorSessionConfigFactory(es, func(es *EvalSessionState) core.Builder {
	...
	br.Register("report_suite_summary", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := evaluatorSessionConfigFactory(es, func(es *EvalSessionState) core.Builder {
	...
	br.Register("next_point", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := NextPointFactory(es)
		return factory(def, vars)
	})
	br.Register("run_point", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := RunPointFactory(es)
		return factory(def, vars)
	})
	br.Register("report_session", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		es := initESS()
		factory := ReportSessionFactory(es)
		return factory(def, vars)
	})

	br.Register("create_point_dir", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CreatePointDirBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("copy_sample_workspace", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CopySampleWorkspaceBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("copy_sample_docs", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CopySampleDocsBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("init_workspace_repo", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &InitWorkspaceRepoBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("stage_workspace_baseline", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &StageWorkspaceBaselineBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("commit_workspace_baseline", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CommitWorkspaceBaselineBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("run_agent", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &RunAgentBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("run_oracle_check", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &RunOracleCheckBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("collect_trace_tokens", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CollectTraceTokensBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("check_agent_version", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CheckAgentVersionBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("summarize_point_results", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &SummarizePointResultsBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("collect_metrics", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
		return &CollectMetricsBuilder{ES: &initESS().EvalState}, nil
	})
	br.Register("dump_config", func(def catalog.ToolDef, vars map[string]string) (core.Builder, error) {
