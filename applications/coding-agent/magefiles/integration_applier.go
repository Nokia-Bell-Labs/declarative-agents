...
// its HTTP contracts against the shipped profile (srd006 R2, R3, R4; rel08.0
// uc001). It is the tracer test-rel08.0-applier names as `mage integration:applier`.
//
...
// point: this runs the declaration that ships, and a word dropped from a machine
// or an argv contract changed in exec-declarations.yaml fails here.
//
...
// this is evidence about the machines, the arguments they construct, and the
// responses they map -- not about a cluster. The chart schema is proven
// separately against real helm (applier_schema_test.go), and the live tier is
...
func (Integration) Applier() error {
	...
	// The tracer needs the agent-core checkout, the Go toolchain, git, and sh
	// (the recording fakes are shell scripts on the applier's PATH).
	if reason := baseIntegrationSkipReason(roots, "sh"); reason != "" {
		fmt.Printf("SKIP applier: %s\n", reason)
		return nil
	}
	return runApplierIntegration(roots)
}

func runApplierIntegration(roots integrationRoots) error {
	...
	traceDir, err := os.MkdirTemp("", "coding-agent-applier-traces-*")
	if err != nil {
		return err
	}
	...
	if err := waitServingHTTP(applierControlHealthURL, applierReadyTimeout); err != nil {
	...
	fmt.Println("integration:applier PASS - the shipped applier walked every apply and rollout leg, " +
		"constructed the declared helm and kubectl arguments, verified all three role Deployments, " +
...
// startApplierAgent launches the shipped applier profile as a detached server
// with the recording fakes ahead of the real tools on its PATH, and its
// workspace set to the fakes' work directory so write_overrides lands inside it.
func startApplierAgent(binary, coreRoot, profilesRoot string, fakes *applierFakes, tracePath string) (*applierAgent, error) {
	profile := filepath.Join(profilesRoot, "applications", "coding-agent", "applier", "profile.yaml")
	...
	cmd := exec.Command(binary,
		...
		"--otel-service-name", "coding-applier",
	)
...
// the verify declare (exec-declarations.yaml). The coding applier has no state
// endpoint, so there is no `helm get values` leg.
