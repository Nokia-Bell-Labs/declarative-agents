...
// These cover the applier's rollout read (srd002-applier R3.3). `kubectl rollout
// status` exits non-zero both for a rollout that is still progressing and for one
// it could not read at all -- an absent Deployment, a wrong kubeconfig, a denied
...
// carries the counts the rollout response declares. The read targets the
// collector, the application's stable server, not the bounded curator.
...
// TestApplierRolloutEndpointMapsThreeOutcomes proves the three terminal states map
// to three responses: a broken read answers 502, and both live phases carry the
// counts the rollout response declares.
...
// the machine's declared terminal states are the same set.
...
// TestApplierRolloutCountsWordContract proves the counts word targets the installed
// release's collector Deployment placeholder the chart rewrites and carries the
// full contract metadata, errors and relationships included.
func TestApplierRolloutCountsWordContract(t *testing.T) {
	...
	if !containsString(word.Args, "deployment/agent-architecture-collector") {
		t.Errorf("args %v do not name the collector Deployment placeholder the chart rewrites", word.Args)
	}
...
// Deployment shapes a rollout passes through and proves each renders parseable JSON
