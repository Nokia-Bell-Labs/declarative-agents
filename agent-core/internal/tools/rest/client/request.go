...
func applyAuth(req *http.Request, auth AuthProfile, resolver CredentialResolver) error {
	...
	case "", authNone:
		return nil
	case authBearer:
		token, err := credentials.Resolve(resolver, auth.TokenRef)
		if err != nil {
			return err
		}
	...
	case authBasic:
		username, err := credentials.Resolve(resolver, auth.UsernameRef)
		if err != nil {
			return err
		}
		password, err := credentials.Resolve(resolver, auth.PasswordRef)
		if err != nil {
			return err
		}
