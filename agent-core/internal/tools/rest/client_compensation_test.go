...
func TestRESTClient_ChromaAddFreshUndoUsesConfiguredDelete(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, ok)
	require.Equal(t, "collection-1", compensation.Parameters["collection"])
	require.Equal(t, "record-1", compensation.Parameters["ids"])
	require.Equal(t, "delete_records", compensation.Compensation["operation"])
