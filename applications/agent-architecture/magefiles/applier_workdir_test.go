...
// definitions and in tool declarations -- so one mounted profile parameterizes per
// pod and the write and the read cannot drift apart at packaging time.
//
// They must be asserted together, because when they drift nothing reports it:
// write_overrides writes one path, helm reads another, and helm renders whatever
// the release already had. Worse, if the written path falls outside the agent's
// workspace the write is refused outright and every apply dies on its first word.
//
// The chart pins workspace.workDir to /work (the schema rejects any other value),
// so the mount does not move; what these prove is that both the deployment env and
// the mounted profile derive the work path from one reference that resolves to
// /work.

// applierValuesPath returns the values-file path the apply endpoint seeds, with
// placeholders left as their declared defaults.
...
// writes is the path the helm words read.
