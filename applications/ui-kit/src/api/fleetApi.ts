...
// The fleet view (GET /monitor/fleet, catalog srd021 R3 over chatbot-mesh
// srd008 R3): the observer's command-state view of its monitor fan-in joins
// and Kubernetes inventory, reduced to one card per discovered pod. Moved from
// the observer UIs of chatbot-mesh, cohere-demo, and agentic-wiki-mesh, which
// were identical apart from cohere-demo's typed machine and annotation fields.

export const FLEET_PATH = "/monitor/fleet";
export const OBSERVER_STATE_PATH = "/monitor/state";
...
export type FleetAgent = {
  ...
  machine?: {
    ...
    transitions?: Array<{ state?: string; signal?: string; next?: string; action?: string }>;
  };
...
// fetchFleet reads the observer's own fleet view; the observer serves the UI,
// so the read is same-origin rather than proxied.
export async function fetchFleet(client: KitClient): Promise<FleetData> {
  return fleetData(await client.getJSON<FleetResponse>(FLEET_PATH));
}

// fetchObserverState enriches the status bar and never fails a fleet poll.
export async function fetchObserverState(client: KitClient): Promise<string> {
  try {
    const body = await client.getJSON<{ run?: { state?: string } }>(OBSERVER_STATE_PATH);
