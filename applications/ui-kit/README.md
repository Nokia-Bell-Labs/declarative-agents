...
Later releases follow the same pattern: replace both `1.0.0` occurrences with the release version, for example `1.1.0`.

## Panels

A panel is a component plus a manifest (srd004 R4). Each lives in `src/panels/<Name>/` as `<Name>.tsx`, its scoped stylesheet, `manifest.ts`, and `index.ts`, which exports the view component, the mountable component, and `definePanel(manifest, mount)`. `definePanel` rejects a manifest that names an endpoint outside the presentation contract. A mountable component receives `PanelProps`: the ui.yaml panel `config`, the declared `monitoredAgents`, and the declared `traceBackend`. A trace backend is an agent name read through `/monitor-proxy/{agent}/`, or a same-origin path prefix starting with `/` (`/` is the origin root); the shell takes the prefix of `trace_backend.query_path` when it is set, else `trace_backend.name` (`traceBackendFromConfig`). `TraceView` takes optional `openTraceId` and `onOpen` so an application can drive the open trace from the URL. `kitPanelRegistry` keys every published panel by manifest id.
