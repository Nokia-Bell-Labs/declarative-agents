...
**Bench convergence reports.** The evaluation UI shows types as colored badges (green Clean, yellow Recovery, orange Stuck, red Divergent); filtering to one type isolates a failure mode: only Stuck runs to diagnose cycling, only Recovery runs to measure correction cost. Side-by-side model views render the distributions as stacked bars with token-cost overlays, so a shift toward Clean reads as improvement and toward Stuck as regression at a glance.

**CI regression gates.** A fixed task suite runs against each profile commit and the type distribution is compared to a baseline; alerts fire when a type moves beyond threshold (Clean down >5%, Stuck up >3%), catching prompt degradations that binary pass rates miss before they accumulate into visible failures.
